...
// for every bit we should check how many numbers have set bit and maintain the max count as it's asked that largest combination must be greater than 0 


for(int bitPosition=0;bitPosition<24;bitPosition++){
    int count=0;
    for(int i=0;i<n;i++){
        if((1<<bitPosition) & candidates[i]) count++;
    }
