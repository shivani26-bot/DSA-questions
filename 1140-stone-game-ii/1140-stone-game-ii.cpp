...
//     //    if alice choose number of piles greedily , that is if he chooses max number of piles that is x=2 then m=(m,x) ie(1,2) =2 will be the value for bob, and then bob has options 1<=x<=2*2, hence bob will take all the remaining piles.
//     //    hence thinking greedily will not work and it will increase the chances for alice to loose.
//     //    hence if alice choose max number of x every time it will increase range of x for bob as well and bob will end up having more number of options than alice
...
// // we add stoes value only when it's alice turns
// int n=piles.size();
